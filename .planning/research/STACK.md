...
1. `f64` or `f32` for money fields.
2. Postgres/cloud DBs for core ledger state.
3. Excel COM/interop automation for read/write.
4. Replacing Excel with a web-only workflow for CPA handoff.
5. Rules embedded only in Rust code (no runtime-editable rule files).
6. Non-deterministic transaction IDs.
...
cargo add blake3
cargo add rkyv --features std,bytecheck
cargo add rhai --features sync,serde,decimal

# File/system behavior
cargo add notify walkdir globset
