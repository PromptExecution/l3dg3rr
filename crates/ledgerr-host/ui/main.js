...
function buildUI(){
  try{
    ...
    PANELS.forEach(function(p,i){
      ...
      pc.appendChild(div);
      btn.addEventListener('click',function(){PANELS.forEach(function(p2,j){var d=document.getElementById('panel-'+p2.id);if(d)d.classList.toggle('hidden',j!==i);});document.querySelectorAll('.nav-item[data-panel-index]').forEach(function(b,j){b.classList.toggle('active',j===i)})});
    });
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  bar.id='autorun-bar';
  bar.style.cssText='position:fixed;bottom:0;left:0;right:0;background:#203647;color:#e7f0f8;text-align:center;padding:8px;font-size:14px;z-index:99999;font-family:monospace;white-space:pre-wrap;border-top:2px solid #4a90d9';
  document.body.appendChild(bar);
  bar.textContent='[autorun] init';
  buildUI();
  setTimeout(function(){
    var api=window.__TAURI__;
    if(!api){bar.textContent='[autorun] no __TAURI__';return}
    api.core.invoke('get_test_harness_config').then(function(cfg){
      var ver=cfg.pkg_version||'?';
      var bld=cfg.build_number||'?';
      var total=cfg.kill_delay_ms||5000;
      var start=Date.now();
      bar.textContent='[v'+ver+'+b'+bld+'] ';
      document.title='ledgrrr v'+ver+'+b'+bld;
      var tick=setInterval(function(){
        var rem=Math.max(0,total-(Date.now()-start));
        bar.textContent='[v'+ver+'+b'+bld+'] '+(rem/1000).toFixed(1)+'s';
        document.title='ledgrrr v'+ver+'+b'+bld+' ['+(rem/1000).toFixed(1)+'s]';
        if(rem<=total*0.3&&!window.___dumped){
          window.___dumped=true;
          var walker=document.createTreeWalker(document.body,4);
          var texts=[];
          while(walker.nextNode()){var t=walker.currentNode.textContent.trim();if(t)texts.push(t);}
          var dump=texts.join('\n').slice(0,5000);
          dump='[v'+ver+'+b'+bld+']\n'+dump;
          api.core.invoke('write_dom_dump',{dump:dump}).catch(function(){});
        }
        if(rem<=0){clearInterval(tick);bar.textContent+=' [exiting]';setTimeout(function(){window.close()},300);}
      },200);
    }).catch(function(e){bar.textContent='[autorun] err: '+e.message});
  },1000);
});
